...
export const zh = {
  ...
  home: {
    welcome: "欢迎参与本研究",
    intro:
      "您将在大约 15–20 分钟内审核若干患者消息及对应的 AI 草稿回复，并决定如何处理。所有案例均改编自真实数据库（已脱敏，不包含可识别身份的信息）。",
    notMedical:
  ...
  consent: {
    title: "知情同意",
    p1: "您将审核若干患者消息以及 AI 起草的回复，并对每条作出处理决定。所有案例均改编自真实数据库（已脱敏，不包含可识别身份的信息）。",
    p2: "我们将记录您的审核选择、最终回复文本、点击事件与时长。所有数据按假名 ID 脱敏存储，仅用于学术研究。",
    p3: "您可以随时关闭页面退出研究。完成研究后会显示一个完成码（completion code）。",
    p4: "本研究的参与报酬取决于您的认真程度。如果注意力检查题答错、或回复明显敷衍（如随意复制、与案例无关），将无法获得参与报酬。",
    checklistIntro:
      "在审核每条 AI 草稿时，请您自行留意以下核查角度（正式案例中不再逐条列出清单）：",
    checklistBullets: [
      "所选处理方式与患者风险程度是否匹配（含是否需升级或线下评估）。",
      "回复是否避免无依据的安抚，并包含必要的应急提示与就医引导，以及警示症状（需立即就医的体征）说明。",
