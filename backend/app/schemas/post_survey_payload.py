...
它挡下的是这些情况：
    量表题填了 0 分或 6 分（超出 1~5 的范围）
    ★ 注意力检查题没按要求选 4（说明在一路点到底）
    三道开放题只填了空格
    手机尾号填了 "abcd" 或只填 3 位
    多传了不该有的字段（说明前后端题目对不上）

--------------------------------------------------------------------------------
★ 加一道新题必须同时改三个地方，缺一不可
--------------------------------------------------------------------------------
  1. frontend/src/lib/forms/postSurveyConfig.ts  前端显示这道题
  2. 这个文件                                     后端接受这个字段
  3. backend/data/post_survey.json                题库存档

只改第 1 个的话：医生能看到、能填，但一点提交就被这里挡下来报 422，
而且他完全不知道为什么。

--------------------------------------------------------------------------------
★ 字段名不能改
--------------------------------------------------------------------------------
字段名 = 数据库 JSON 里的键名 = 导出 CSV 的列名 = 论文编码表的变量名。
四者必须一致，改了名已收集的数据就和研究工具对不上了。

--------------------------------------------------------------------------------
本文件的代码块（从上到下）：
--------------------------------------------------------------------------------
  第 1 块  _L                          所有量表题共用的"必须 1~5 分"约束
  第 2 块  POST_SURVEY_LIKERT_KEYS     全部量表题的字段名列表
  第 3 块  PostSurveyV7Payload         ★ 校验规则本体
  第 4 块  post_survey_v7_all_threes() 测试用：生成一份合规的假答案
================================================================================
"""
...
# ── 第 1 块：量表题的公共约束 ────────────────────────────────────────────
# ge=1, le=5 表示"必须大于等于 1、小于等于 5"。
# 提出来复用，省得在下面写 40 多遍一模一样的 Field(ge=1, le=5)。
_L = Field(ge=1, le=5)

# ── 第 2 块：全部量表题的字段名 ──────────────────────────────────────────
# 只有第 4 块那个生成测试数据的函数用得到它。
# 单独列一份而不是从下面的类里反射取，是为了改题时能一眼看全。
