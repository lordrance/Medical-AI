"""
================================================================================
文件作用：一次性的演示脚本 —— 造一份完整假数据，验证导出功能没坏
================================================================================
...
★ 这不是生产代码，也不是测试。它是一个"手动跑一下看看"的演示脚本：

  1. 在临时目录里建一个全新的空数据库（不碰你的开发库，更不碰生产库）
  2. 灌进题目，模拟一位医生走完全程：建档 → 前测 → 8 道题 → 后测
  3. 调用导出功能，把 ZIP 解开，逐个检查每张表的 CSV 是不是都有内容

★ 什么时候用得上：改动了导出相关的代码之后，跑一下这个脚本，
  能直观看到"导出的 ZIP 里到底有哪些文件、每个文件里有多少行"。
  自动化测试只会告诉你通过或失败，这个脚本会把内容打印出来给你看。

★ 完全安全：数据库建在系统临时目录里，跑完就可以删。
"""
